...
mrcpp::MultiResolutionAnalysis<D> setup_mra(int oper_scale) {
    ...
    auto world = mrcpp::BoundingBox<3>(0, corner, boxes, s_fac, true);
    ...
    // int reach = -oper_scale;//mrcpp::math_utils::ipow(2, -oper_scale + 1);
    // println(0, "reach " << reach);
    // MRA.setOperatorScale(0);
    // MRA.setPeriodicOperatorReach(reach + 1);
    // MRA.setPeriodicOperatorCutOff(reach);
    // MRA.setOperatorScale(oper_scale);
...
*/
    mrcpp::Timer t0;
    mrcpp::PoissonOperator P(MRA, prec, -n, reach);
    t0.stop();
    ...
    if (energies.size() > 0) d_energy = n_energy - energies.front();
    ...
    */
    ...
    if (g_trees.size() > 0) {
        ...
        mrcpp::add(-1.0, d_tree, 1.0, *g_tree, -1.0, *g_trees.front());
